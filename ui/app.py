...
def _narration_subscriber():
    ...
    while True:
        try:
            ...
            # Poll with get_message(timeout=...) instead of the blocking listen():
            # it returns None on an idle tick (no exception), so the connection
            # stays alive between negotiations and never drops events.
            while True:
